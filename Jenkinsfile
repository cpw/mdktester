pipeline {
    ...
    stages {
        ...
        stage('buildandtest') {
            steps {
                sh './gradlew ${GRADLE_ARGS} --refresh-dependencies -PJENKINS_MC_VERSION=${MC_VERSION} -PJENKINS_MCP_VERSION=${MCPVERSION} -PJENKINS_FORGE_VERSION=${MDKVERSION} --continue build test'
            }
            post {
                always {
                    discordSend(
